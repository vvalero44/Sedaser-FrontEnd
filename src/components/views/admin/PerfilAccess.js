...
export const PerfilAccess = () => {
  ...
  const GetWorks = async (user) => {
    try {
      const getProfileUrl = `${env.backend}/api/admin/GetUserWorks/${params.userId}`;
      const config = {
        headers: {
  ...
  const PostWorks = async () => {
    try {
      const getProfileUrl = `${env.backend}/api/admin/PostEdithUserWork/${params.userId}`;
      const config = {
        headers: {
  ...
  const DeleteWork = async (id) => {
    if (window.confirm("¿Seguro que quieres eliminar el trabajo?")) {
      try {
        const getProfileUrl = `${env.backend}/api/admin/DeleteEdithUserWork/${id}`;
        const config = {
          headers: {
            Authorization: `Bearer ${userLogged.token}`,
            "x-access-token": userLogged.token,
