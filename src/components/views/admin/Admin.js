...
export const AdminView = (user) => {
  ...
  const GetUsers = async (user) => {
    try {
      const getProfileUrl = `${env.backend}/api/admin/ListUsers`;
      const config = {
        headers: {
  ...
  const GetAdmins = async (user) => {
    try {
      const getProfileUrl = `${env.backend}/api/admin/ListAdmins`;
      const config = {
        headers: {
          Authorization: `Bearer ${user.token}`,
          "x-access-token": user.token,
  ...
  const PostUser = async () => {
    try {
      const getProfileUrl = `${env.backend}/api/auth/signup`;
      const config = {
        headers: {
  ...
  const DeleteUser = async (id) => {
    if (window.confirm("¿Seguro que quieres eliminar el trabajo?")) {
      try {
        const getProfileUrl = `${env.backend}/api/admin/DeleteEdithUser/${id}`;
        const config = {
          headers: {
            Authorization: `Bearer ${userLogged.token}`,
            "x-access-token": userLogged.token,
